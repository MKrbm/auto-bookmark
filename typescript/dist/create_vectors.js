 ...
 */
export async function create_vectorsMain(docs, filename, userTitle) {
    try {
        // 1) テキスト分割
        //    doc => { pageContent: doc.page_content } の形に合わせる
        const allSplits = await textSplitter.splitDocuments(docs.map(doc => ({
            pageContent: doc.page_content,
            metadata: doc.metadata
        })));
        // 2) チャンクごとに Embedding
        const embeddings = await Promise.all(allSplits.map(chunk => embedding_model.embedDocuments([chunk.pageContent])));
